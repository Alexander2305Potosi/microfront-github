    ...
    <div class="bg-gray-800 rounded-2xl border border-gray-700 overflow-hidden shadow-xl">
      ...
      <div class="p-6 border-b border-gray-700 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p class="text-sm text-gray-400 mt-1" *ngIf="subtitle">{{ subtitle }}</p>
        </div>
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
          <thead>
            <tr class="bg-gray-900/50 border-b border-gray-700 text-gray-300 text-sm font-semibold uppercase tracking-wider">
              <th *ngFor="let col of columns" class="px-6 py-4">{{ col.label }}</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-700/50 bg-gray-800">
            <tr *ngFor="let row of filteredData" class="hover:bg-gray-750 transition-colors group">
              <td *ngFor="let col of columns" class="px-6 py-4 whitespace-nowrap">
                ...
                <ng-container *ngIf="col.type === 'avatar'">
                  <img [src]="row[col.key]" class="w-10 h-10 rounded-full ring-2 ring-gray-700 group-hover:ring-blue-500 transition-all">
                </ng-container>
                ...
                <ng-container *ngIf="col.type === 'link'">
                  <a [href]="row[col.key]" target="_blank" class="text-blue-400 hover:text-blue-300 font-medium transition-colors">
                    Ver enlace &rarr;
                  </a>
                </ng-container>
      ...
      <!-- Pagination -->
      <div *ngIf="enablePagination && filteredData.length > 0" class="px-6 py-4 border-t border-gray-700 bg-gray-900/30 flex items-center justify-between">
        <span class="text-sm text-gray-400">
          Mostrando <span class="font-medium text-gray-200">1</span> a <span class="font-medium text-gray-200">{{ filteredData.length }}</span> de <span class="font-medium text-gray-200">{{ data.length }}</span> resultados
        </span>
        <div class="flex space-x-2">
          <button class="px-3 py-1 rounded-md bg-gray-800 border border-gray-600 text-gray-300 hover:bg-gray-700 transition-colors disabled:opacity-50" disabled>Anterior</button>
          <button class="px-3 py-1 rounded-md bg-gray-800 border border-gray-600 text-gray-300 hover:bg-gray-700 transition-colors">Siguiente</button>
        </div>
      </div>
    </div>
  `
...
export class CoreDataTableComponent {
  ...
  @Input() columns: Array<{key: string, label: string, type?: 'text'|'avatar'|'badge'|'link'}> = [];
